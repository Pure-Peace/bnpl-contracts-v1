...
contract BNPLBankNode is Initializable, AccessControlEnumerableUpgradeable, IBNPLBankNode {
     ...
     */
    event LoanApproved(address indexed borrower, uint256 loanRequestId, uint256 loanId, uint256 loanAmount, address operator);
    ...
    IERC20 public baseLiquidityToken;// = IERC20(0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD);
    IMintableTokenUpgradeable public poolLiquidityToken;// = IMintableToken(0x63801824694C6a0482C7416Ea255D6a3745F82f2);

    IERC20 public bnplToken;// = IERC20(0x1d1781B0017CCBb3f0341420E5952aAfD9d8C083);
    
    uint16 public unusedFundsLendingMode;
    IAaveLendingPool public unusedFundsLendingContract;
    IERC20 public unusedFundsLendingToken;

    IBNPLSwapMarket public bnplSwapMarket;// = IBNPLSwapMarket(0x121E2e269fD5B33cc6a381EA81E1A6D7ec142692);
    uint24 public bnplSwapMarketPoolFee;
    
    uint32 public bankNodeId;
    
    IBNPLNodeStakingPool public nodeStakingPool;// = IBNPLNodeStakingPool(0x2643be3DaeD4566B42150B291f75D19B1a23098E);
    IBankNodeManager public bankNodeManager;
    

    uint256 public baseTokenBalance;
    uint256 public nodeOperatorBalance;
    uint256 public accountsReceivableFromLoans;
    uint256 public poolTokensCirculating;
    
    uint256 public loanRequestIndex;
    ...
    mapping(uint256 => uint256) public interestPaidForLoan;
    mapping(uint256 => uint256) public loanBondedAmount;
    ...
    function initialize(BankNodeInitializeArgsV1 calldata bankNodeInitConfig)
        ...
        initializer
    {
        require(bankNodeInitConfig.unusedFundsLendingMode == 1,"unused funds lending mode currently only supports aave (1)");
        ...
        poolLiquidityToken = IMintableTokenUpgradeable(bankNodeInitConfig.poolLiquidityToken);
    ...
    function calculateSlashAmount(uint256 prevNodeBalance, uint256 nodeLoss, uint256 poolBalance) public pure returns (uint256){
        uint slashRatio = PRBMathUD60x18.div(nodeLoss*PRBMathUD60x18.scale(),prevNodeBalance*PRBMathUD60x18.scale());
        return (poolBalance*slashRatio)/PRBMathUD60x18.scale();
    }
    
    function getMonthlyInterestPayment(uint256 loanAmount, uint256 interestAmount, uint256 numberOfPayments, uint256 currentMonth) public pure returns(uint256) {
        return loanAmount*PRBMathUD60x18.mul(getPrincipleForMonth(interestAmount, numberOfPayments, currentMonth-1), interestAmount)/PRBMathUD60x18.scale();
    }
    function getLoanNextDueDate(uint256 loanId) public view returns (uint64){
        ...
        uint256 nextPaymentDate = (uint256(loan.numberOfPaymentsMade+1)*uint256(loan.totalLoanDuration)/uint256(loan.numberOfPayments))+uint256(loan.loanStartedAt);
        return uint64(nextPaymentDate);
        
        
    }
    function getPrincipleForMonth(uint256 interestAmount, uint256 numberOfPayments, uint256 currentMonth) public pure returns(uint256) {
        uint ip1 = (PRBMathUD60x18.scale()+interestAmount);
        uint ip1m = PRBMathUD60x18.pow(ip1,currentMonth);
        uint pin = getPaymentMultiplier(interestAmount, numberOfPayments);
        uint rightFrac=PRBMathUD60x18.div(ip1m-PRBMathUD60x18.scale(), interestAmount);
        uint right = PRBMathUD60x18.mul(pin, rightFrac);
        return ip1m-right;
    }
    function getMonthlyPayment(uint256 loanAmount, uint256 interestAmount, uint256 numberOfPayments) public pure returns(uint256) {
    ...
    function getPaymentMultiplier(uint256 interestAmount, uint256 numberOfPayments) public pure returns(uint256) {
        uint ip1 = (PRBMathUD60x18.scale()+interestAmount);
        uint ip1n = PRBMathUD60x18.pow(ip1,numberOfPayments);
        uint top = PRBMathUD60x18.mul(interestAmount,ip1n);
        uint bottom = (ip1n-PRBMathUD60x18.scale());
        uint result = PRBMathUD60x18.div(top,bottom);
        return result;
    }
    ...
    function _removeLiquidity(address user, uint256 poolTokensToConsume) private returns (uint256) {
        ...
        require(poolTokensToConsume != 0 && poolTokensToConsume <= poolTokensCirculating, "poolTokenAmount cannot be 0 or more than circulating");
    ...
    function _requestLoan(address borrower, uint256 loanAmount, uint64 totalLoanDuration, uint32 numberOfPayments, uint256 interestRatePerPayment, uint8 messageType, string memory message) private {
        require(loanAmount <= MAX_LOAN_AMOUNT);
        require(loanAmount >= MIN_LOAN_AMOUNT);
        require(interestRatePerPayment > 0);
        
        uint amountPerPayment = getMonthlyPayment(loanAmount, interestRatePerPayment, numberOfPayments);
        
        require(loanAmount <= (amountPerPayment*uint256(numberOfPayments)), "payments not greater than loan amount!");
        require(((totalLoanDuration/uint256(numberOfPayments))*uint256(numberOfPayments))==totalLoanDuration, "totalLoanDuration must be a multiple of numberOfPayments");
        require(totalLoanDuration>=MIN_LOAN_DURATION, "must be greater than MIN_LOAN_DURATION");
        require((uint256(totalLoanDuration)/uint256(numberOfPayments))>=MIN_LOAN_PAYMENT_INTERVAL, "must be greater than MIN_LOAN_PAYMENT_INTERVAL");
    ...
    function requestLoan(uint256 loanAmount, uint64 totalLoanDuration, uint32 numberOfPayments, uint256 interestRatePerPayment, uint8 messageType, string memory message) public override {
        _requestLoan(msg.sender, loanAmount, totalLoanDuration, numberOfPayments, interestRatePerPayment, messageType, message);
    }
    ...
    function _approveLoanRequest(address operator, uint256 loanRequestId) private {
        ...
        require(loanAmount <= (loanRequest.amountPerPayment*uint256(loanRequest.numberOfPayments)), "payments not greater than loan amount!");
        require(((loanRequest.totalLoanDuration/uint256(loanRequest.numberOfPayments))*uint256(loanRequest.numberOfPayments))==loanRequest.totalLoanDuration, "totalLoanDuration must be a multiple of numberOfPayments");
        require(loanRequest.totalLoanDuration>=MIN_LOAN_DURATION, "must be greater than MIN_LOAN_DURATION");
        require((uint256(loanRequest.totalLoanDuration)/uint256(loanRequest.numberOfPayments))>=MIN_LOAN_PAYMENT_INTERVAL, "must be greater than MIN_LOAN_PAYMENT_INTERVAL");
    ...
    function _markLoanAsWriteOff(uint256 loanId) private {
        ...
        require(loan.borrower != address(0));
        require(loan.loanStartedAt<uint64(block.timestamp),"cannot make the loan payment on same block loan is created");
        ...
        uint256 slashAmount = calculateSlashAmount(prevBalanceEquivalent, prevBalanceEquivalent-getPoolTotalAssetsValue(), poolBalance);
    ...
    function _makeLoanPayment(address payer, uint256 loanId) private {
        ...
        uint currentPaymentId = loan.numberOfPaymentsMade;
        ...
        uint interestAmount = getMonthlyInterestPayment(loan.loanAmount, loan.interestRatePerPayment, loan.numberOfPayments, loan.numberOfPaymentsMade+1);
        uint holdInterest = interestAmount/5;
        //uint returnInterest = interestAmount - holdInterest;
        uint bondedInterest = holdInterest/2;
        uint marketBuyInterest = holdInterest-bondedInterest;
        
        uint amountPerPayment = loan.amountPerPayment;
